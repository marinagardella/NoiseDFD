...
def get_characters(img, blob_thresh=30):
    """
    Detect characters in an image using multi-Otsu thresholding.
    """
...
def detect_three_channels(img_path, res_path):
    """
    Detect characters in three channels of an image and save masks.
    """
    ...
    characters_path = f"{res_path + '/' + sub_folder}/characters.png"
    io.imsave(characters_path, 255 * labels_binary.astype(np.uint8))
    for ch in range(3):
        stds = []
        img_ch = img[:, :, ch]
        ...
        mask_ch_path = (
            f"{res_path + '/' + sub_folder}/mask_{ch}.png"
        )
        io.imsave(mask_ch_path, mask_ch.astype(np.uint8))

    io.imsave(f'{res_path + '/' + sub_folder}/mask.png', mask.astype(np.uint8))


def compute_NFA(img_path, res_path, east_model_path):
    """
    Compute the NFA for detected characters in an image.
    """
    sub_folder = os.path.splitext(os.path.basename(img_path))[-2]
    ...
    io.imsave(
        f'{res_path + '/' + sub_folder}/text-mask.png', 
    ...
    if not os.path.exists(res_path + '/' + sub_folder):
        os.makedirs(res_path + '/' + sub_folder)
    io.imsave(
        f'{res_path + "/" + sub_folder}/nfa.png',
        255 * NFADets.astype(np.uint8)
    )
...
if __name__ == "__main__":
    ...
    for img in Path(path).glob('*.tif'):
        ...
        if (
            os.path.exists(os.path.join(output_path, sub_folder, 'mask.png'))
            and os.path.exists(
                os.path.join(output_path, sub_folder, 'characters.png')
            )
        ):
            print(
                f"Skipping char comp for {img} results already exist.")
            compute_NFA(str(img), output_path, east_model_path)
        else:
