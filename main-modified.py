...
def detect(img_path, alpha, threshold, output_path):
    ...
    nfas = np.zeros_like(img)
    for i in range(3):
        labeled_objects, labels = extract_components(img[:,:,i])
        chars = (labeled_objects > 0)
        stds, stds_img = compute_std_per_label(img[:,:,i], labeled_objects, labels)
        ...
        text_mask = detect_text(img[:,:,i], east_model_path)
        nfa = compute_NFA(outliers, chars, text_mask, threshold, alpha)
        nfas[:,:,i] = nfa
        sub_folder = os.path.splitext(os.path.basename(img_path))[-2]
        if not os.path.exists(output_path + sub_folder + str(i)):
            os.makedirs(output_path + sub_folder + str(i))
        cv2.imwrite(output_path + sub_folder + str(i) + '/characters.png', chars * 255)
        cv2.imwrite(output_path + sub_folder + str(i) + '/outliers.png', outliers)
        cv2.imwrite(output_path + sub_folder + str(i) + '/words.png', text_mask)
        cv2.imwrite(output_path + sub_folder + str(i) + '/nfa.png', nfa * 255)
        cv2.imwrite(output_path + sub_folder + str(i) + '/stds.png', stds_img * 255)
    final_nfa = nfas[: , : , 0]*nfas[: , : , 1]*nfas[: , : , 2]
    if not os.path.exists(output_path + sub_folder):
            os.makedirs(output_path + sub_folder )
    cv2.imwrite(output_path + sub_folder + '/nfa.png', final_nfa * 255)
